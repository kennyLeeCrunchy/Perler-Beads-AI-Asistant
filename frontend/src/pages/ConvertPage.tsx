...
import { generatePatternBundle, resolveApiMediaUrl, type PatternBundleResult, type PatternResult } from '../api';
...
async function transformImage(file: File, mode: 'crop' | 'rotate'): Promise<File> {
  ...
  if (mode === 'rotate') {
    ...
    context.drawImage(source, (source.width - square) / 2, (source.height - square) / 2, square, square, 0, 0, square, square);
  }
  const blob = await new Promise<Blob>((resolve, reject) => canvas.toBlob((result) => result ? resolve(result) : reject(new Error('图片处理失败')), 'image/png'));
  return new File([blob], file.name.replace(/\.[^.]+$/, '') + `-${mode}.png`, { type: 'image/png' });
}
...
export function ConvertPage() {
  ...
  const [originalFile, setOriginalFile] = useState<File | undefined>(routeState.image);
  ...
  const [bundle, setBundle] = useState<PatternBundleResult>();
  const [work, setWork] = useState<Work>();
  const uploaded = Boolean(imageFile || routeState.sourcePath || routeState.cleanReferenceId);
  ...
  const applyImageTransform = async (mode: 'crop' | 'rotate') => {
    if (!imageFile) return;
    try {
      const transformed = await transformImage(imageFile, mode);
      chooseImage(transformed);
    } catch (reason) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const result = await generatePatternBundle({
        image: imageFile,
        sourcePath: imageFile ? undefined : routeState.sourcePath,
        cleanReferenceId: routeState.cleanReferenceId,
        brand,
        maxColors,
        similarityThreshold: similarity,
        useTransparentMask: removeBg,
      });
      const selectedVariant = result.variants.find((item) => item.width === gridSize) ?? result.variants[0];
      if (!selectedVariant) throw new Error('后端没有返回图纸结果');
      const hexByCode = new Map(selectedVariant.counts.map((item) => [item.code, item.hex]));
      const realGrid = selectedVariant.cells.flat().map((code) => code ? hexByCode.get(code) ?? null : null);
      const realPalette: BeadColor[] = selectedVariant.counts.map((item) => ({
        brand,
        code: item.code,
        name: item.name,
        hex: item.hex,
      ...
      const nextWork: Work = {
        id: `pattern-${selectedVariant.pattern_id}`,
        name: fromAI ? 'AI 生成拼豆图纸' : (imageFile?.name.replace(/\.[^.]+$/, '') || '新拼豆图纸'),
        size,
        gridSize,
        board,
        brand,
        colors: selectedVariant.counts.length,
        beads: selectedVariant.counts.reduce((total, item) => total + item.count, 0),
        status: '草稿',
        createdAt: now,
        updatedAt: now,
        progress: 0,
        palette: realPalette,
        grid: realGrid,
        patternId: selectedVariant.pattern_id,
        previewDataUrl: selectedVariant.preview_data_url,
        cleanReferenceId: result.clean_reference_id ?? undefined,
  ...
  const selectVariant = (variant: PatternResult) => {
    const nextSize = `${variant.width}×${variant.height}` as typeof allowedSizes[number];
    setSize(nextSize);
    setPattern(variant);
    if (!work) return;
    const hexByCode = new Map(variant.counts.map((item) => [item.code, item.hex]));
    const nextGrid = variant.cells.flat().map((code) => code ? hexByCode.get(code) ?? null : null);
    const nextPalette = variant.counts.map((item) => ({ brand, code: item.code, name: item.name, hex: item.hex, count: item.count, remaining: item.count }));
    const nextWork = { ...work, size: nextSize, gridSize: variant.width, grid: nextGrid, colors: variant.counts.length, beads: variant.counts.reduce((sum, item) => sum + item.count, 0), palette: nextPalette, patternId: variant.pattern_id, previewDataUrl: variant.preview_data_url };
    setWork(nextWork);
    upsertWork(nextWork);
  };
  ...
  return <ResponsivePageContainer className="page-section">
    ...
    <div className="convert-layout">
      <section className="image-panel panel-card">
        ...
        {uploaded && <div className="preview-tools"><Button variant="ghost" icon={<Crop size={17} />} onClick={() => applyImageTransform('crop')} disabled={!imageFile}>裁剪</Button><Button variant="ghost" icon={<RotateCw size={17} />} onClick={() => applyImageTransform('rotate')} disabled={!imageFile}>旋转</Button><Button variant="ghost" icon={<RotateCcw size={17} />} onClick={resetImage} disabled={!originalFile}>重置</Button><Button variant="secondary" icon={<ImageUp size={17} />} onClick={clearImage}>更换图片</Button></div>}
      </section>
      <section className="parameters-panel panel-card">
        ...
        <div className="form-grid"><label className="field-stack"><span>品牌色卡</span><select value={brand} onChange={(event) => setBrand(event.target.value as 'Artkal' | 'Mard')}><option value="Artkal">Artkal M 系列</option><option value="Mard">Mard 221 色</option></select><small>量化、色号和用量统计都会使用所选品牌。</small></label><label className="field-stack"><span>最大颜色数量</span><select value={maxColors} onChange={(event) => setMaxColors(Number(event.target.value))}><option value={12}>12 色</option><option value={20}>20 色（推荐）</option><option value={24}>24 色</option><option value={36}>36 色</option><option value={0}>根据色卡自动</option></select></label></div>
    ...
    {status === 'done' && pattern && work && <section className="conversion-result panel-card"><div className="result-preview"><span className="success-badge"><Check size={17} />同源三档图纸生成完成</span><BeadGrid size={pattern.width} cells={grid} colors={palette} /></div><div className="result-summary"><span className="eyebrow">转换结果</span><h2>{work.name}</h2><p>算法 {work.algorithmVersion ?? 'bead_ready_52_v2'} · AI 清稿 {bundle?.ai_passes ? '1 次' : '未使用'}。选择任意尺寸进入编辑器。</p>{pattern.quality && <div className="info-note quality-note"><strong>质量门禁</strong><span>结构 {pattern.quality.structure.score}/100 · 颜色重建 {pattern.quality.color.score}/100 · CIEDE2000 已启用</span></div>}<div className="variant-picker">{bundle?.variants.map((variant) => <button key={variant.pattern_id} className={variant.pattern_id === pattern.pattern_id ? 'is-active' : ''} onClick={() => selectVariant(variant)}><strong>{variant.width}×{variant.height}</strong><small>{variant.counts.length} 色 · {variant.counts.reduce((sum, item) => sum + item.count, 0)} 颗</small></button>)}</div><PatternSummary work={work} /><div className="dominant-colors"><span>主色</span>{palette.slice(0, 8).map((color) => <i key={color.code} style={{ background: color.hex }} title={`${color.code} · ${color.count} 颗`} />)}</div><div className="result-buttons"><Button variant="secondary" onClick={() => setStatus('ready')}>重新调整参数</Button><Button onClick={() => navigate(`/editor/${work.id}`)}>进入图纸编辑器 <ArrowRight size={17} /></Button></div></div></section>}
