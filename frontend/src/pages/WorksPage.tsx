...
export function WorksPage() {
  ...
  const filtered = useMemo(() => works.filter((work) => (filter === '全部' || work.status === filter) && work.name.includes(query)).sort((a, b) => sort === '名称' ? a.name.localeCompare(b.name, 'zh-CN') : sort === '图纸尺寸' ? (b.gridSize ?? 0) - (a.gridSize ?? 0) : sort === '最近创建' ? +new Date(b.createdAt) - +new Date(a.createdAt) : +new Date(b.updatedAt) - +new Date(a.updatedAt)), [works, filter, query, sort]);
  ...
  const rename = () => {
    if (!active) return;
    const nextName = window.prompt('请输入新的作品名称', active.name)?.trim();
    if (!nextName || nextName === active.name) return;
    const next = { ...active, name: nextName, updatedAt: new Date().toISOString() };
    const list = works.map((work) => work.id === active.id ? next : work);
    setWorks(list);
    saveWorks(list);
    setActive(next);
    showToast('作品名称已更新');
  ...
  return <ResponsivePageContainer className="page-section works-page">
    ...
    <Drawer open={!!active} title="作品详情" onClose={() => setActive(null)}>{active && <div className="work-detail"><div className="detail-preview">{active.grid.length && active.gridSize ? <BeadGrid size={active.gridSize} cells={active.grid} colors={active.palette} mirror={active.mirror} /> : active.previewDataUrl ? <img src={active.previewDataUrl} alt="" /> : <BeadGrid motif={active.motif} size={20} />}</div><div><span className={`status-pill ${active.status === '已完成' ? 'done' : ''}`}>{active.status}</span><h2>{active.name}</h2><PatternSummary work={active} compact /><p>最后编辑：{new Date(active.updatedAt).toLocaleDateString('zh-CN')}</p></div><div className="detail-actions"><Button onClick={() => navigate(`/editor/${active.id}`)}>继续编辑</Button><Button variant="secondary" onClick={() => navigate(`/make/${active.id}`)}>制作辅助</Button></div><div className="drawer-menu"><button onClick={() => navigate(`/make/${active.id}`)}><Download />导出图纸</button><button onClick={duplicate}><Copy />复制作品</button><button onClick={rename}><Edit3 />重命名</button><button className="danger" onClick={() => setConfirmDelete(true)}><Trash2 />删除作品</button></div></div>}</Drawer>
