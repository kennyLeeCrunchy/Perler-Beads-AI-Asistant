...
export function EditorPage() {
  ...
  return <main className="editor-page">
    <header className="editor-command-bar"><div className="editor-title"><IconButton onClick={() => navigate('/works')} aria-label="返回作品"><ArrowLeft size={20} /></IconButton><input value={name} onChange={(e) => { setName(e.target.value); setDirty(true); }} /><span className={dirty ? 'dirty' : ''}>{dirty ? '有未保存修改' : '已保存'}</span></div><div className="editor-center-actions"><IconButton onClick={undo} disabled={!history.length}><Undo2 /></IconButton><IconButton onClick={redo} disabled={!future.length}><Redo2 /></IconButton><button className={`text-tool ${mirror ? 'is-active' : ''}`} onClick={() => setMirror(!mirror)}><FlipHorizontal2 />{mirror ? '镜像' : '正面'}</button><span className="zoom-label">100%</span></div><div className="editor-main-actions"><Button variant="secondary" icon={<Save size={17} />} onClick={save}>保存</Button><Button onClick={() => navigate(`/make/${baseWork.id}`)}>制作辅助</Button><IconButton className="editor-more" onClick={() => setDrawerOpen(true)}><MoreHorizontal /></IconButton></div></header>
    <div className="editor-workspace"><EditorToolbar active={tool} onChange={setTool} onUndo={undo} onRedo={redo} /><section className="canvas-area"><CoordinateBoard size={gridSize} mirror={mirror}><BeadGrid motif={baseWork.motif} size={gridSize} cells={baseGrid} colors={baseWork.palette} selected={selected} highlightedColor={highlighted} mirror={mirror} showNumbers displayMode="pattern" overrides={overrides} onCellClick={onCell} onCellHover={(x, y) => setCoordinate(`X ${x} / Y ${y}`)} /></CoordinateBoard><div className="canvas-hint">点击或框选编辑 · Ctrl + 点击多选</div></section><aside className="editor-properties panel-card"><div className="panel-tabs"><button className={tab === 'color' ? 'is-active' : ''} onClick={() => setTab('color')}>颜色</button><button className={tab === 'selection' ? 'is-active' : ''} onClick={() => setTab('selection')}>选区</button><button className={tab === 'info' ? 'is-active' : ''} onClick={() => setTab('info')}>信息</button></div>{tab === 'color' && <><div className="brand-row"><span>当前色卡</span><strong>{baseWork.brand}</strong></div><ColorList colors={baseWork.palette} active={activeColor.hex} onSelect={(color) => { setActiveColor(color); setHighlighted(color.hex); }} /><div className="property-actions"><Button variant="secondary" onClick={() => setReplaceOpen(true)}>替换全图同色</Button><Button variant="ghost" onClick={() => setHighlighted(undefined)}>取消高亮</Button></div></>}{tab === 'selection' && <div className="selection-panel"><div className="selection-hero"><strong>{selected.size}</strong><span>个格子已选中</span></div><div className="metric-row"><div><small>选区宽高</small><strong>{selected.size ? '4 × 3' : '—'}</strong></div><div><small>当前颜色</small><strong>{activeColor.code}</strong></div></div><Button fullWidth onClick={() => fillSelection(activeColor.hex)}>填充当前颜色</Button><Button fullWidth variant="secondary" onClick={() => fillSelection(null)}>擦除选区</Button><Button fullWidth variant="ghost" onClick={() => setSelected(new Set())}>清除选择</Button></div>}{tab === 'info' && <PatternSummary work={baseWork} />}</aside></div>
    <footer className="editor-status"><span>{coordinate}</span><span>当前色号 {activeColor.code}</span><span>缩放 100%</span><span>已选 {selected.size} 格</span><span>{baseWork.brand} 色卡</span></footer>
